...
3. **Преобразование в криптографическую затравку:**  
   ...
   - $seed_p = H_{\mathrm{SHA256}}\big(\text{derived\_key}_{[:32]} \parallel "p"\big)$

   - $seed_q = H_{\mathrm{SHA256}}\big(\text{derived\_key}_{[32:]} \parallel "q"\big)$
...
- **Безопасность:**  
  ...
  Высокая чувствительность к начальному состоянию гарантирует, что даже незначительное изменение входных данных приводит к существенному изменению латентного представления автоэнкодера.
...
- **Результаты:**  
  ...
  За 5 эпох первичного обучения функция потерь (val_loss) снижается примерно до 0.117.
