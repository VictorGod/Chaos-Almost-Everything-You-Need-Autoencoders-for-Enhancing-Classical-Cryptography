Ниже приведена полная версия документации, включающая подробное описание системы, карт хаотичности, результатов тестирования и сводную таблицу оценки результатов.
...
- **Структура:**  
  ...
  - **Латентный слой:** Полносвязный слой из 64 нейронов, использующий ту же активацию и дополненный кастомным регуляризатором VarianceRegularizer:
    $$ L = -\lambda \log(\operatorname{Var}(z) + \epsilon) $$
  
Результатом работы энкодера является 64-мерное латентное представление, которое используется при генерации криптографических ключей.
...
3. **Преобразование в криптографическую затравку:**  
   ...
   - $$ \text{seed}_p = H_{\mathrm{SHA256}}\Big(\text{derived\_key}_{[:32]} \; \Vert \; "p"\Big) $$
   - $$ \text{seed}_q = H_{\mathrm{SHA256}}\Big(\text{derived\_key}_{[32:]} \; \Vert \; "q"\Big) $$
...
- **Генерация обучающих данных:**  
  Использование логистического отображения и отображения Арнольда позволяет синтезировать большое количество уникальных изображений с высокой энтропией.
- **Источники энтропии:**  
  Хаотические карты вместе с системными случайными данными (из `os.urandom`, timestamp и др.) обеспечивают криптографическую затравку для генерации RSA‑ключей.
- **Безопасность:**  
  Высокая чувствительность к начальным условиям гарантирует, что даже незначительное изменение входных данных приводит к существенному изменению латентного представления автоэнкодера.

---

## 4. Обучение и динамическое дообучение

### 4.1 Первичное обучение

- **Датасет:**  
  Для обучения используются изображения, сгенерированные логистическим отображением:  
  $$ x_{n+1} = 3.99 \cdot x_n \cdot (1 - x_n) $$
  Дополнительно могут применяться преобразования, такие как отображение Арнольда, для повышения хаотичности.
- **Результаты:**  
  За 5 эпох первичного обучения функция потерь снижается примерно до 0.117 (val_loss).

### 4.2 Динамическое (экспресс) дообучение

Перед выполнением шифрования/дешифрования выполняется дообучение автоэнкодера для повышения энтропии ключей:

- **Методы дообучения:**
  - Дообучение на случайных картах хаотичности, сгенерированных логистическим отображением.
  - Дополнительное дообучение на уникальных случайных изображениях, полученных из данных `os.urandom`.
  
- **Параметры:**  
  Используется оптимизатор Adam с learning rate \(10^{-4}\) и небольшое количество эпох (обычно 2).
...
## 5. Сводная таблица оценки результатов

Ниже представлена сводная таблица, объединяющая показатели криптографических процессов, автоэнкодера и соответствие стандартам. Таблица включает итоговые экспериментальные данные и комментарии.

| Метрика | Ваше решение | Эталон / Типичные значения | Статус | Комментарий |
|---------|--------------|----------------------------|--------|-------------|
...
- **Запуск:**  
  Для выполнения основного скрипта введите:
  ```bash
  python -m venv venv
